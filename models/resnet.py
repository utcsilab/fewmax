...
class ResNet(nn.Module):
    ...
    def forward_backbone(self, x, output_features=['layer4']):
        ...
        if 'layer0' in output_features:
            features['layer0'] = f0
        f0 = self.maxpool(f0)
        f1 = self.layer1(f0)
        if 'layer1' in output_features:
            features['layer1'] = f1
        f2 = self.layer2(f1)
        if 'layer2' in output_features:
            features['layer2'] = f2
        f3 = self.layer3(f2)
        if 'layer3' in output_features:
            features['layer3'] = f3
        f4 = self.layer4(f3)
        if 'layer4' in output_features:
            features['layer4'] = f4
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        ) if k in state and state[k].size() == v.size()}
